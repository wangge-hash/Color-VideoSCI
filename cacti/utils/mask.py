...
def generate_masks(mask_path=None,mask_shape=None):
    ...
    # mask2 = 2*mask-1
    # mask2_s = np.sum(mask2*mask2, axis=0)
    # mask_s = np.sum(mask, axis=0)
    # mask_s[mask_s==0] = 1
    return mask0, mask0_s, mask1, mask1_s

def generate_masks_real(mask_path=None,mask_shape=None):

    mask0 = scio.loadmat(mask_path[0])['mask']
    mask1 = scio.loadmat(mask_path[1])['mask']
    # mask = mask['mask']
    if mask_shape is not None:
        c,h,w = mask0.shape
        ...
        h_b = np.random.randint(0,h-m_h+1)
        w_b = np.random.randint(0,w-m_w+1)
        mask0 = mask0[:m_c,h_b:h_b+m_h,w_b:w_b+m_w]
        mask1 = mask1[:m_c,h_b:h_b+m_h,w_b:w_b+m_w]
