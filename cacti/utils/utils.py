...
def load_checkpoints(model,pretrained_dict,strict=False):
    ...
    if strict is True:
        ...
        temp_pretrained_dict = {}
        for k in pretrained_dict: 
            if k not in model_dict.keys():
                print("layer: {} not find!".format(k))
                continue
            temp_pretrained_dict[k]=pretrained_dict[k]
            if model_dict[k].shape != pretrained_dict[k].shape:
                temp_pretrained_dict[k] = model_dict[k]
...
def save_image_color(out,work_dir,meas_name):
    c,f,h,w = out.shape
    for i in range(f):
        cv2.imwrite(work_dir + '/' + meas_name.split('.')[0] + '_{}.png'.format(i),out.transpose(2,1,3,0)[:,i,:,::-1]*255.)    
