...
class DavisMixData(Dataset):
    def __init__(self,data_root,*args,**kwargs):
        ...
        self.gene_meas_G = build_pipeline({k:v[0] for k,v in kwargs["gene_meas"].items()})
        self.gene_meas_C = build_pipeline({k:v[1] for k,v in kwargs["gene_meas"].items()})
    ...
    def __getitem__(self,index):
        ...
        gt0,meas0 = self.gene_meas_G(imgs,self.mask[0])
        gt1,meas1 = self.gene_meas_C(imgs,self.mask[1],self.rgb2raw)
    
        return gt0, gt1, meas0, meas1
