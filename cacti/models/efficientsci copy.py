...
class ResDNetBlock0(nn.Module):
    def __init__(self,dim,group_num):
        ...
        for i in range(group_num):
            self.cformer_list.append(CFormerBlock0(group_dim))
            if i > 0:
...
class ResDNetBlock1(nn.Module):
    def __init__(self,dim,group_num):
        ...
        self.dense_conv = nn.ModuleList()
        for i in range(group_num):
            self.cformer_list.append(CFormerBlock1(group_dim))
            if i > 0:
                self.dense_conv.append(
                    nn.Sequential(
                        nn.Conv3d(group_dim*(i+1),group_dim,1),
...
class EfficientSCI0(nn.Module):
    def __init__(self,in_ch=64, units=8,group_num=4,color_ch=1):
        ...
        for i in range(units):
            self.resdnet_list.append(ResDNetBlock0(in_ch*4,group_num=group_num))
...
class EfficientSCI1(nn.Module):
    def __init__(self,in_ch=64, units=8,group_num=4,color_ch=1):
        ...
        self.resdnet_list = nn.ModuleList()
        for i in range(units):
            self.resdnet_list.append(ResDNetBlock1(in_ch*4,group_num=group_num))
...
@MODELS.register_module 
class EfficientSCI2(nn.Module):
    def __init__(self,in_ch=64, units=8,group_num=4,color_ch=1):
        super().__init__()
        self.color_ch = color_ch
        self.fem = nn.Sequential(
            nn.Conv3d(3, in_ch, kernel_size=(3,7,7), stride=1,padding=(1,3,3)),
            nn.ELU(),
            nn.Conv3d(in_ch, in_ch*2, kernel_size=3, stride=1, padding=1),
            nn.ELU(),
            nn.Conv3d(in_ch*2, in_ch*4, kernel_size=3, stride=(1,2,2), padding=1),
    ...
    def forward(self,y_list,Phi_list,Phi_s_list):
        out_list = []
        y0,y1,y2 = y_list
        Phi0,Phi1,Phi2 = Phi_list
        Phi0_s,Phi1_s,Phi2_s = Phi_s_list
        
        meas0_re = torch.div(y0, Phi0_s)
        meas1_re = torch.div(y1, Phi1_s)
        meas2_re = torch.div(y2, Phi2_s)
        # meas_re = torch.unsqueeze(meas_re, 1)
        maskt0 = Phi0.mul(meas0_re)
        maskt1 = Phi1.mul(meas1_re)
        maskt2 = Phi2.mul(meas2_re)
        x0 = maskt0
        x0 = x0.unsqueeze(1)
        x1 = maskt1
        x1 = x1.unsqueeze(1)
